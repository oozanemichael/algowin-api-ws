...
import org.mh.exchange.binance.dto.*;
...
public class BinanceStreamingMarketDataService implements StreamingMarketDataService {
  ...
  private final org.mh.exchange.binance.BinanceStreamingService service;
  ...
  public BinanceStreamingMarketDataService(
      org.mh.exchange.binance.BinanceStreamingService service,
  ...
  public Observable<OrderBook> getOrderBook(ParsingCurrencyPair currencyPair, Object... args) {
    if (!service.getProductSubscription().getOrderBook().contains(currencyPair.getCurrencyPair())) {
      throw new UnsupportedOperationException(
          "Binance exchange only supports up front subscriptions - subscribe at connect time");
    }
    return orderbookSubscriptions.get(currencyPair.getCurrencyPair());
  }
  ...
  public Observable<Ticker> getTicker(ParsingCurrencyPair currencyPair, Object... args) {
    return getRawTicker(currencyPair.getCurrencyPair()).map(BinanceTicker24h::toTicker);
  }

  @Override
  public Observable<Trade> getTrades(ParsingCurrencyPair currencyPair, Object... args) {
    return getRawTrades(currencyPair.getCurrencyPair(), args)
        .map(
            rawTrade ->
                new Trade.Builder()
